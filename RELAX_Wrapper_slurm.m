...
for FileNumber=RELAX_cfg.FilesToProcess(1,1:size(RELAX_cfg.FilesToProcess,2))
    ...
    try
        ...
        if RELAX_cfg.computecleanedmetrics==1    
            ...
            % IMK note: after GEDAI, RELAX SER/ARR will be biased and harder to interpret. Ignore these metrics.
            EEG=continuousEEG;
            EEG = rmfield(EEG,'RELAXProcessing');
        ...
        if EEG.etc.GEDAI.percentage_rejected > 80
            EEG.RELAX_issues_to_check.GEDAI_percent_epochs_rejected_high=EEG.etc.GEDAI.percentage_rejected;
        else
            EEG.RELAX_issues_to_check.GEDAI_percent_epochs_rejected_high=0;
        end
        ...
        % Handle case where cleaning for one file errors
        % This often happens for noisy files where GEDAI removes too many epochs
        warning('File %d (%s) could not be cleaned and was skipped. Error: %s', ...
            FileNumber, RELAX_cfg.filename, ME.message);

        continue  % move to the next FileNumber in this participant's file list
    end
end
...
clearvars -except 'RELAX_cfg' 'FileNumber' 'CleanedMetrics' 'RawMetrics' 'RELAXProcessingMWFStepOneAllParticipants' 'RELAXProcessingMWFStepTwoAllParticipants' 'RELAXProcessing_wICA_AllParticipants'...
        'RELAXProcessing_ICA_AllParticipants' 'RELAXProcessingMWFStepThreeAllParticipants' 'Warning' 'RELAX_issues_to_check' 'RELAX_issues_to_check_2nd_run'...
        'RELAXProcessingExtremeRejectionsAllParticipants' 'WarningAboutFileNumber' 'GEDAI_ShortFileSkips';
